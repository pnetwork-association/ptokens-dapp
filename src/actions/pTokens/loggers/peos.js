...
const peosLoggedIssue = (_ptokens, _params, _dispatch) => {
  ...
  _ptokens.peos.issue(..._params)
    .once('onEosTxConfirmed', _tx => {

      const amount = parseFloat(_tx.processed.action_traces[0].act.data.quantity).toFixed(4)
...
const peosLoggedRedeem = (_ptokens, _params, _dispatch) => {
  ...
  _ptokens.peos.redeem(..._params)
    .once('onEthTxConfirmed', _tx => {

      const explorer = `${settings.peos.eth.etherscanLink}tx/${_tx.transactionHash}`

      const message = `Burn Transaction confirmed! ${parseFloat(_params[0]).toFixed(4)} pEOS Burnt`
