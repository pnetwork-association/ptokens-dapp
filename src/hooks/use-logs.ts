...
const useEvents = (chain: Chain) => {
  ...
  const getLogs = async () => {
    ...
    const allLogs = (await Promise.all(Object.values(swapChains).map((chain) => retreiveLogs(chain.chainId, 100000)))).flat()
  ...
  useEffect(() => {
    try {
      ...
      const intervalId = setInterval(() => !isLoading.current && getLogs(), 60000)
