...
const redeemPTokens = async ({amount, recipient, userData, ptokenFrom, ptokenTo, progress }: {amount: string; recipient: string; userData: string; ptokenFrom: pTokensAsset; ptokenTo: pTokensAsset; progress: TProgressContext;}) => {
  ...
  if (ptokenFrom.assetInfo.chain !== Chain.BscMainnet) {
    try {
