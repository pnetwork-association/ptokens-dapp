...
const loadEvmCompatibleBalances =
  (_obj: { assets: AssetWithAddress[]; account: string; blockchain: Blockchain }) => (_dispatch: AppDispatch) => {
    _obj.assets.map((asset) => _dispatch(loadEvmCompatibleBalance({ asset, account, blockchain })))
  }

const loadEvmCompatibleBalance =
  (_obj: { asset: AssetWithAddress; account: string; blockchain: Blockchain }) => async (_dispatch: AppDispatch) => {
    try {
      ...
      const balance = (await token.methods.balanceOf(account).call()) as string
